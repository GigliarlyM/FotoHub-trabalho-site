...
function Photo({ titulo, url, price, className = "", promo = 0 }) {
    return (
        <div className={className}>
            <p style={{textAlign: 'center', fontSize: 24}}>{titulo}</p>
            <img src={url} width={200} />
            {promo ?
                <p style={{textAlign: 'center', fontWeight: "bold"}}>R$ {price - (price * promo)} com - {promo * 100}%</p> :
                <p style={{textAlign: 'center', fontWeight: "bold"}}>R$ {price}</p>
            }
        </div>
    )
}
